...
import asyncio
import re
from typing import List, Dict, Any, Callable, Optional, Union, Tuple, get_type_hints
...
class Agent:
    ...
    async def set_project_context(self, project_id: str, session_id: str = "default"):
        ...
        NOTE: Memory functionality has been removed. This method is now a no-op.
        """
        if self.debug:
            print(f"[Agent] set_project_context called but memory is disabled")
        return
            ...
            try:
                ...
                if self.telemetry_enabled:
                    try:
                        ...
                        # Incrementally track message chars (only count new messages since last iteration)
                        non_system = [m for m in session.messages if m.get("role") != "system" and m.get("role") != "assistant"]
                        if len(non_system) > _last_msg_count:
                            _message_chars += sum(len(str(m.get("content", ""))) for m in non_system[_last_msg_count:])
                            _last_msg_count = len(non_system)
                        output_chars = len(str(content or ""))

                        breakdown = TokenBreakdown(
                            system_instructions=_system_chars // 4,
                            tool_definitions=_tools_chars // 4,
                            messages=_message_chars // 4
                        )
                        
                        input_tokens = (_system_chars + _message_chars + _tools_chars) // 4
