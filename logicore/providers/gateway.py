...
Architecture:
  Agent (provider-agnostic)
        ...
        │  chat() / chat_stream()
        ▼
  ProviderGateway / ResilientGateway
...
Resilient Features:
  ...
  - Mid-stream retry for streaming failures
...
# =========================================================================
# Resilient Gateway — Automatic retry, failover, and health tracking
# =========================================================================
...
class ResilientGateway(ProviderGateway):
    ...
    Usage:
        ...
        gateway = ResilientGateway(provider)
        
        # With availability service for multi-provider failover
    ...
    def __init__(
        ...
        from .policies import RetryPolicy, DEFAULT_RETRY_POLICY
        self.retry_policy = retry_policy or DEFAULT_RETRY_POLICY
        
        # Get the underlying gateway for the provider
        self._inner_gateway = get_gateway_for_provider(provider)
        
        # Track current provider for failover
    ...
    async def _try_failover(self) -> bool:
        ...
        Returns:
            True if successfully switched to another provider, False otherwise
        """
        ...
        # Find next available provider after current
        current_found = False
    ...
    async def chat(
        ...
        """Chat with automatic retry and failover."""
        ...
        while True:
            ...
            # Try with current provider using retry policy
            async for attempt in RetryIterator(
                ...
                try:
                    ...
                    # Classify and report failure
                    category = None
                    if self.availability:
                        category = self.availability.report_failure(provider_id, e)
                    
                    # Check if we should retry with current provider
                    try:
    ...
    async def chat_stream(
        ...
        """Streaming chat with retry, mid-stream retry, and failover."""
        from .policies import RetryIterator
        ...
        while True:
            ...
            async for attempt in RetryIterator(
                ...
                try:
                    # Buffer for potential mid-stream retry
                    accumulated_content = ""
                    
                    # Wrapper to capture content for retry
                    ...
                    # Check for mid-stream retry (partial content received)
                    if self.enable_stream_retry and stream_retries < self.max_stream_retries:
                        ...
                        if is_stream_error:
                            ...
                            # Exponential backoff for stream retries
            ...
            if await self._try_failover():
                if self._current_provider_id not in providers_tried:
                    stream_retries = 0  # Reset stream retries for new provider
                    continue
...
def get_resilient_gateway(
    ...
    **kwargs
    ...
    Args:
        ...
        availability: Optional availability service for multi-provider failover
        retry_policy: Optional custom retry policy
        **kwargs: Additional arguments for ResilientGateway
    
    Returns:
        A configured ResilientGateway instance
    """
